...
export function initialize(apiKey?: string, model: string = 'gemini-2.5-flash', baseUrl?: string) {
    const key = apiKey || process.env.API_KEY;
    if (!key) {
        // If we are in an environment where the key is injected via other means (like a proxy without auth), 
        // we might proceed, but standard SDK usage requires a key.
        // We'll let the SDK throw if it's missing.
        console.warn("No API Key provided to initialize.");
    }
